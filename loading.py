...
def check_data_loss(train_path="train.csv", test_path=None, num_method=None, cat_method=None, threshold=10): 
    try:
        ...
        if dropped_percent > threshold:
            return False  # Return None to indicate failure
        # Xử lý test nếu có
        if test is not None:
            if num_method != "drop":
...
def loading(path="train.csv",feature_columns=None, num_method=None, cat_method=None, target_column=None, time_series=False, time_column=None, special_columns=None):
    try:
        ...
        if feature_columns:
            ...
            if missing:
                raise ValueError(f"The following feature columns are not found in the {path} dataset: {missing}")
            train = train[feature_columns + [target_column]]

        print("Train data loaded successfully!")

        if special_columns is not None: #Special columns are columns that need to be filled with forward fill
        ...
        train = train.dropna(subset=[target_column]) #Make sure target column has no missing values because if otherwise, it will cause errors in model training
        before = len(train)
        # Xử lý missing data
...
def regression_model(cat_cols=None, time_series=False, num_cols=None, log_transform=False, n_iter=20, cv=10):
    # Choose CV strategy
    cv = TimeSeriesSplit(n_splits=cv) if time_series else cv
...
def detect_class_type(y, class_threshold=15):
    ...
    if dtype == "object" or dtype == "category":
        ...
        if num_classes > 2:
            print(f"Multi-class classification detected.Unique values: {num_classes}")
            return "multiclass"
        else:
